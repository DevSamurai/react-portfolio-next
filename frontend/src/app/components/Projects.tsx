        ...
        <div className="container mx-auto max-w-4xl p-4 py-12">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {projects.map(async (project, index) => (
              <div
                key={index}
                className={`group relative h-52 cursor-default rounded-lg col-span-1 bg-cover bg-center md:col-span-${project.span}`}
                style={{
                  backgroundImage: `url(${
                    (await getStrapiMediaUrl(
                      project.picture?.data?.attributes?.formats?.medium?.url
                    )) || ""
                  })`,
                }}
              >
